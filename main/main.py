...
# Creation of Nb boundary points on [-1,1]x[0,0]/Creation of Nc collocation points on [-1,1]x[0,1]
def prep_data(Nb, Nc):
...
def dif_loss(model, X_c, T_c, max):  # partial derivatives and equation
    ...
    u = model(XX, TT)
    du_dx = grad(u, XX, torch.ones_like(
        XX), create_graph=True, retain_graph=True)[0]
    du_dxx = grad(du_dx, XX, torch.ones_like(
        XX), create_graph=True, retain_graph=True)[0]
    du_dxxx = grad(du_dxx, XX, torch.ones_like(
        XX), create_graph=True, retain_graph=True)[0]

    du_dt = grad(u, TT, torch.ones_like(
        TT), create_graph=True, retain_graph=True)[0]
